...
! ntransitions
  ...
  ! of the elements and their ionisation stages given in compose_adata.dat
  DO 
     ! Read only the first line of every element in particular ionisation stage
     ...
     ! ionisation potential will not change (or increase), e.i. ion.pot. increment ionoffset = 0.
     IF (a_numb .NE. remember_last_element) ionoffset = 0.
     ! Flag for the deciding for which elements of given ionisation stage we will use data 
     ! For flag = 0. we will only read data but we will not use tham, othervise we will use them
     flag = 0
...
!        PRINT*, 'n_ions=', n_ions
        ...
        DO J = 1, n_ions                   
          ...
          IF ((a_numb .EQ. atomicnumber) .AND. (i_stat .EQ. ionstage)) THEN 
              ! Associate the pot.energy i_pot (multiplie with e_v) from compose_adata.dat
...
!              print*, I, J, n_lev
              ! Allocete the memory; define how large should be elements(I)%ions(J)%levels (n_lev)
              ALLOCATE (elements(I)%ions(J)%levels(n_lev))
              ! Loop over all lavels 
              DO K = 1, n_lev
                 READ(8,*) l_numb, l_e, s_waight, n_tran
!                PRINT*, l_numb, l_e, s_waight, n_tran
                 ! Encrease the level energy l_e (multiplie with e_v) by ionoffset i.e. 
                 ! for the ion.pot. of the graund level
...
!                IF (K .EQ. 2) PRINT*,  l_e * e_v + ionoffset
              ...
              END DO
              ! When the all levels of the given element in the particular ionisation stage
	...
	END DO
      ...
      ! In case we did not find the given element in the particular ionisation stage
...
10 CONTINUE
  ...
  ! Real number of transitions (how many transitions are involved)
  ntransitions = ntransitions/2.
