...
INTEGER, DIMENSION(2)                   :: dummy_var, dummy
...
!!!!!!!!!!!!!!!!!!!!!!!
! ZATÍM JEN VG A JE IMPLEMENTOVÁN
!!!!!!!!!!!!!!!!!!!!!!!
...
DO cur_point = 1, n_modelgrid
 ...
 cur_n_r_B = floor(cur_r/w_vgrid_r + 1.0/2.0) + 1
 cur_n_t_B = floor(cur_t/w_vgrid_t + 1.0/2.0) + 1
...
write(*,*) 'connect_2D_peku: vg_indexy_A = ', vg_indexy_A(:,2)
write(*,*) '**************************************************'
...
DO cur_point = 2, n_modelgrid
 ...
 dummy_var = vg_indexy_A(cur_point,:)

 DO WHILE(I > 1)
  IF(vg_indexy_A(I,1) > dummy_var(1)) THEN
   dummy = vg_indexy_A(I + 1,:)
...
END DO
write(*,*) 'connect_2D_peku: indices_A = ', indices_A
