 SUBROUTINE event_dist(pack_index, cell_dist, e_dist, event)
  ...
  IMPLICIT NONE    
    ...
    DOUBLE PRECISION                  :: e_dist, ran_numb, ran2, tau_rand, cell_dist
...
!  For now we neglect cont. opacities, but the routine was written generally to allow for adding
!  cont. opacity in the future
   electron_density = 0.D0 !take this from modelgrid finally
   kappa_cont = sigma_e * electron_density

    DO WHILE (do_loop .EQ. 1) 
       IF (package(pack_index)%freq_cmf .GT. freq_line) THEN
!          Calculate distance the photn needs to travel to coming to resonance with the next line
!          This assumes homologus expansion i.e. velocity is proportional to r 
!          Projected gradient of the projected velocity in a direction of the photon propagation is more complicated in case of no homologus expannsion
...
!                        In this case tha package propagates to the next cell
...
!               Continuum process will happens
           ...
           END IF
       ELSE    
!          The package cmf frequency is too red to interact to another line  - No line interact anymore 
