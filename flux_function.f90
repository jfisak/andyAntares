...
CASE(0)
 !write(*,*) 'expr = ', const_h * freq / (BOLK * temperature)
 IF( (const_h * freq / (BOLK * temperature)) < 7.D2) THEN
  flux_function = ( 2.D0 * const_h * freq**3 / light_speed**2 )  * &
   1.D0 / ( EXP( (const_h * freq / (BOLK * temperature) ) - 1.D0 ) )
 ELSE
  flux_function = 0.D0
 END IF
! diluted blackbody
CASE(1)
 ...
 ! temperature = 1.E4
 IF( (const_h * freq / (BOLK * temperature)) < 7.D2) THEN
  flux_function = W * ( 2.D0 * const_h * freq**3 / light_speed**2 )  * &
   1.D0 / ( EXP( (const_h * freq / (BOLK * temperature) ) - 1.D0 ) )
 ELSE
  flux_function = 0.D0
 END IF
