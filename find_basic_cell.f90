...
n_basic_cells = nx_cell * ny_cell * nz_cell
...
DO I = 1, 6
 IF(temp_cell(I) > 0 .AND. temp_cell(I) <= n_basic_cells) THEN
...
!print*, 'find_basic_cell: temp_cell = ', temp_cell(I)
!print*, 'find_basic_cell: crossp = ', crossp(1) - corner(1), crossp(2) - corner(2), crossp(3) - corner(3)
!print*, 'find_basic_cell: width = ', width(1), width(2), width(3)
  IF(crossp(1) - corner(1) >= 0.D0 .AND. abs(crossp(1) - corner(1)) <= width(1) .AND. &
     crossp(2) - corner(2) >= 0.D0 .AND. abs(crossp(2) - corner(2)) <= width(2) .AND. &
     crossp(3) - corner(3) >= 0.D0 .AND. abs(crossp(3) - corner(3)) <= width(3) ) THEN
     next_b_cell = temp_cell(I)
     found = found + 1
!     print*, 'found next cell'
  END IF
 END IF
END DO
...
IF(found > 1) STOP 'find basic cell: more than one basic cells were found  which is now impossible'
