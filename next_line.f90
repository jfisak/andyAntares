...
SUBROUTINE next_line(approximation, pack_index, lastline, next1line, n_eqf_lines)
USE types
IMPLICIT NONE
...
IF (lastline == no_line) THEN
 ...
 DO I = 1, ntransitions
  ...
  IF (package(pack_index)%freq_cmf > linelist(I)%freq) THEN
   ...
   lastline = I - 1
   package(pack_index)%last_line = lastline + 1
   IF(I - 1 == 0) lastline = no_line
 ...
 IF (lastline .EQ. no_line) THEN
  lastline = ntransitions - 1
  next1line = lastline + 1
 END IF
ELSE ! the last line /= no_line
 ...
 DO I = lastline, ntransitions
  ...
  IF(package(pack_index)%freq_cmf > linelist(I)%freq) THEN
   ...
   EXIT
  END IF
  IF(I == ntransitions) lastline = ntransitions
 END DO
END IF

! write(*,*) 'next_line: after next1line = ', next1line

IF(lastline == ntransitions) THEN
