! process a package according to chosen event
! 1.) interaction in line
! 2.) continuum event
!     a) photoionization
...
!        event(INT): number of a selected event
!        actirrates(rrates): rates for transitions
! OUTPUT: NONE
!
! 1x RETURN POINT
!
...
ELSE IF (event .EQ. rpkt_eventtype_continuum) THEN
 ...
 Zphotion = 0.D0
 DO ind_I = 2, n_photcrossect + 1
  Zphotion = Zphotion + actirrates%Lcont(4,ind_I)
 END DO
 Zff = actirrates%Lcont(4,n_photcrossect + 2)
 ! write(*,*) 'do_rpackage_event: Zff = ', Zff
 ZcontTot = Zthomson + Zphotion + Zff

 ! DO ind_I = 1, SIZE(actirrates%Lcont(1,:))
 ...
 IF(rand > summ .AND. rand <= Zphotion + summ) THEN
  DO ind_I = 2, n_photcrossect + 1
   IF(rand >= summ .AND. rand <= actirrates%Lcont(4, ind_I) + summ) THEN
    ...
    indexe = INT(actirrates%Lcont(1,ind_I))
    indexi = INT(actirrates%Lcont(2,ind_I))
    indexl = INT(actirrates%Lcont(3,ind_I))
    ! treshold frequency
    ...
    EXIT
   END IF
   summ = summ + actirrates%Lcont(4,ind_I)
  END DO
 END IF
 !______________________________________________________________________
